...
- Name: MoreValidations.spec.js >> more validatiosn
...
```ts
  ...
  2  | 
  3  | test ('more validatiosn',async({page})=>
  4  | {
  5  |     await page.goto('https://rahulshettyacademy.com/AutomationPractice/');
  6  |     //await page.goto('https://google.com');
> 7  |     await expect(page.locator('#displayed-text')).toBeVisible();
     |     ^ ReferenceError: expect is not defined
  8  |     await page.locator('#hide-textbox').click();
  9  |     await expect(page.locator('#displayed-text')).toBeHidden();
  10 |     
